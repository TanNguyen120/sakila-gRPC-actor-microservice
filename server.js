...
server.addService(actorProto.ActorService.service, {
    ...
    get: (call, callback) => {
        try {
            ...
            model.knexObj.select('first_name', 'last_name').from('actor').where('actor_id', call.request.id).then(rows => {
    ...
    create: (call, callback) => {
        try {
            ...
            model.knexObj('actor').insert({ first_name: call.request.first_name, last_name: call.request.last_name }).then(rows => {
